...
import { closeFullscreenPreview } from '~/core/src/store/actions';
...
export default function FullScreenPreview({ fullScreenMode, zoomStep }:Props) {
  ...
  const { src, alt, width, height, visible } = useSelector((st: GlobalState) => ({
    src: st.preview.src,
    alt: st.preview.alt,
    width: st.preview.width,
    height: st.preview.height,
    visible: st.preview.visible
  }));
